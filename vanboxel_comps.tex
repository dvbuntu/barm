...
\usepackage{xcolor}
\newcommand{\dvb}[1]{\textcolor{teal}{DVB: #1}}  % comments by Danielle
\newcommand{\crp}[1]{\textcolor{olive}{CRP: #1}} % comments by Cristian
\newcommand{\xt}[1]{\textcolor{purple}{XT: #1}}     % comments by Xueying
...
% clarify general ML problem y = f(x) + eps
% motivate what BARN might be good at: broad applicability, architecture search with bayesian stats (look this up), scaling computation with problem difficulty/controlling efforts through priors
...
\dvb{Describe need to have a software library to ensure usability by data science practicitioners.  Cite studies that have libs along with them?  Is there a meta study on this?}
...
\dvb{Finally, we explore how these foundations enable further research into broad Bayesian Additive Regression Models \ldots}
...
%% Use subsections here, as we'll be going into considerable detail
...
\dvb{ Add some background on Basic bayesian stats here, note that we cancel the P(Y,X) in the acceptance ratio}
...
\dvb{Mention BART's posterior uses variance as well?}

\dvb{Remark on BART's priors on terminal node value distribution, b/c BARN lacks something similar: so no direct prior on model output}
...
\dvb{Explain general posterior for BARN here, in very little space, but combine NN optimization as part of process}
...
% some addition info on mutate + train, maybe move this earlier
...
\dvb{quickly mention/show that we can use the residual as replacement of non-current componeents, then gibbs sample}
...
\dvb{Talk about setting up data science library, inspiration from BARTPy and BART R implementation.  Usage of sklearn for simplicity.  Inclusion of unit tests, and development of documentation.}
